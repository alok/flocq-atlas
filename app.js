...
const colors = {done:'#25a672',broken:'#e66770',progress:'#6297f3',matched:'#dfa940',unaddressed:'#fff'};
const labels = {done:'Reviewed',broken:'Needs work',progress:'In progress',matched:'Matched',unaddressed:'Unaddressed'};
const statusGuide = {
  ...
  broken: {short:'Open issue or proof obligation', detail:'Red: a known issue or open proof obligation needs attention. This does not necessarily mean the implementation is incorrect.'},
  ...
  matched: {short:'Counterpart found; awaiting review', detail:'Yellow: a likely Lean counterpart has been found, but it has not been marked reviewed in this map.'},
  unaddressed: {short:'No counterpart found by the matcher', detail:'White: no Lean match was found. The declaration may be missing, renamed, or expressed differently.'}
};
...
const unproved = n => n.role==='theorem' && !n.proven;
...
function renderNav(){
  ...
  if(query){
    ...
    html=results.slice(0,100).map(n=>`<button class="module-button search-result ${n.id===selectedId?'active':''}" data-node="${esc(n.id)}"><i class="dot ${n.status}"></i><span class="result-content"><strong>${esc(n.name)}</strong><small>${esc(moduleMap.get(n.module).name)} · L${n.line}${!shown(n)?' · folded':unproved(n)?' · unproved':''}</small></span></button>`).join('');
...
function renderAtlas(){
  ...
  for(const p of panels){
    const folded=scope==='roots'&&p.m.folded?` · +${p.m.folded} folded`:'';
...
// Folded theorems of one module, listed after its last page so none of them is unreachable.
function renderShelf(m,top){
  ...
  hidden.forEach((n,i)=>{
    ...
    const hosts=n.foldedInto.length?n.foldedInto.map(id=>nodeMap.get(id).name).join(', '):'no visible dependency';
    html+=`<g class="fold-chip ${n.id===selectedId?'selected':''}" role="button" tabindex="0" data-node="${esc(n.id)}" aria-label="${esc(n.name)}, folded into ${esc(hosts)}"><title>${esc(n.name)} · folded into ${esc(hosts)}</title><rect x="${x}" y="${y}" width="${width}" height="${chip}" rx="${chip/2}"/><text x="${x+14}" y="${y+chip/2+4}">${esc(label)}</text></g>`;
  });
  return {html,height:60+Math.ceil(hidden.length/columns)*row};
}
function renderGraph(){
  ...
  if(mode==='atlas'){
    ...
    for(const mod of data.modules){
      const p=positions.get(mod.id),list=moduleNodes(mod),counts=Object.fromEntries(Object.keys(labels).map(s=>[s,list.filter(n=>n.status===s).length]));
      let sx=0,bars='';
      for(const s of Object.keys(labels)){const w=(counts[s]/Math.max(1,list.length))*150;if(w){bars+=`<rect x="${p.x+14+sx}" y="${p.y+59}" width="${w}" height="4" fill="${s==='unaddressed'?'#dce3ed':colors[s]}"/>`;sx+=w;}}
      const count=scope==='roots'?`${list.length} shown${mod.folded?` · +${mod.folded} folded`:''}`:`${list.length} declarations`;
      html+=`<g class="node module-node ${mod.id===moduleId?'selected':''}" role="button" tabindex="0" data-module="${esc(mod.id)}" aria-label="Open module ${esc(mod.name)}"><title>${esc(mod.name)} · ${count}</title><rect class="node-box" x="${p.x}" y="${p.y}" width="${p.w}" height="${p.h}" rx="7"/><text x="${p.x+14}" y="${p.y+24}" class="node-title">${esc(mod.name.split('/').at(-1))}</text><text x="${p.x+14}" y="${p.y+43}" class="node-subtitle">${esc(mod.name.split('/')[0])} · ${count}</text><text x="${p.x+p.w-24}" y="${p.y+23}" class="node-number">${String(mod.rank+1).padStart(2,'0')}</text>${bars}</g>`;
    }
    ...
    const withShelf=scope==='roots'&&m.folded&&(!statusFilter||statusFilter==='done');
    ...
    if(withShelf&&page===pages-1){const shelf=renderShelf(m,visibleNodes.length?height-10:30);html+=shelf.html;height=(visibleNodes.length?height-10:30)+shelf.height+20;}
    ...
    $('graph-caption').textContent=`${all.length?`${page*pageSize+1}–${Math.min((page+1)*pageSize,all.length)} of ${all.length}`:'0'} declarations${withShelf?` · ${m.folded} folded after the last page`:''} · scroll to explore`;
  }
...
function selectNode(id,updateHash=true){
  ...
  // Folded theorems sit on the shelf after the module's last page.
...
async function renderInspector(){
  ...
  const candidates=[...n.candidates,...n.related.map(c=>({...c,match:'related open obligation'}))];
  ...
  if(!c){$('lean-code').innerHTML='<div class="source-empty"><strong>No Lean match found</strong>This may be unported, renamed, or handled by different proof infrastructure. White means “not matched in this map,” not a confirmed missing implementation.</div>';$('lean-link').hidden=true;return;}
...
document.addEventListener('click',event=>{
  ...
  const status=event.target.closest('[data-status]');if(status){statusFilter=statusFilter===status.dataset.status?null:status.dataset.status;page=0;
    if(mode==='atlas'&&statusFilter&&nodeMap.get(selectedId)?.status!==statusFilter){
      const first=data.nodes.find(n=>n.status===statusFilter&&shown(n));if(first){selectNode(first.id);locate();return;}
    }
    if(mode!=='atlas'&&statusFilter&&!moduleNodes(moduleMap.get(moduleId)).some(n=>n.status===statusFilter)){
      const first=data.nodes.find(n=>n.status===statusFilter&&shown(n));if(first)selectNode(first.id);
